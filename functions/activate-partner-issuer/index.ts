...
//
// Body: { issuer_id, mode: "verify" | "activate" }
// Auth: Bearer JWT -- MUST be platform_admin.
//
// STEP TWO OF TWO, in two modes.
//
//   mode="verify"    fetch the well-known file, compare the token, set
//                    status='verified'. Repeatable. No key material.
//   mode="activate"  require status='verified', re-check the domain, generate
//                    the Ed25519 keypair, PROVE IT SIGNS, store the private
//                    half in Vault, set status='active'.
//
...
serve(async (req) => {
  ...
  try {
    ...
    if (mode !== "verify" && mode !== "activate") {
      throw new HttpError(400, 'mode must be "verify" or "activate"');
    }

    const { data: issuer, error: iErr } = await svc
      .from("issuers")
      .select(
        "id, slug, name, status, key_id, verification_domain, verification_token, vault_secret_id",
      )
      .eq("id", issuerId)
      .maybeSingle();
    ...
    if (!issuer.verification_domain || !issuer.verification_token) {
      throw new HttpError(422, "issuer has no verification domain or token");
    }
    ...
    if (mode === "verify") {
      ...
      await svc.from("admin_actions").insert({
        ...
        target_id: issuer.id,
        reason: null,
        metadata: { slug: issuer.slug, domain: issuer.verification_domain },
      });
