#!/usr/bin/env node
/**
 * apply-ismsf-batch-1.mjs - the 14 approved ISMS-F descriptions.
 *
 * --apply to write. DRY BY DEFAULT. Unknown flags exit 2.
 *
 * ============ WHAT IS IN, AND WHAT IS NOT ============
 *
 * 13 rows came back `approve` unconditionally. `policy-communication` came
 * back `verify-citation` and PASSED verification against the indexed
 * ISO/IEC 27001:2022 -- clause 5.2 really does ask three separate things
 * (e: documented, f: communicated within the organization, g: available to
 * interested parties). That makes 14.
 *
 * SIX rows are NOT here and are in ISMS-F-REWRITE-BATCH-1-REVISED.json for
 * reading: iso-27005-risk (wording), risk-criteria (needs the definitional
 * half), availability / cryptographic-controls / proportionality (defects),
 * and lessons-learned -- which FAILED citation verification. Its claim named
 * clause 10; the obligation to learn from incidents is Annex A.5.27.
 *
 * ============ THIS SCRIPT WRITES NO HASH COLUMN ============
 *
 * Changing an English description moves `concept_row_en_hash`, which withholds
 * both translations until a generator restamps them from the English it
 * actually translated. That is the gate working, and re-syncing the hash here
 * would blindfold it -- the defect found and repaired earlier today. ISMS-F is
 * blocked in both languages, so nothing serving is affected; the withheld
 * count is reported so the debt is visible rather than silent.
 */
...
const APPROVED = new Set([
  "risk-identification", "threat", "avoid-risk", "physical-perimeter", "nonconformity",
  "roles-segregation", "retain-risk", "scope-statement", "residual-risk",
  "least-privilege", "clear-desk-clear-screen", "effectiveness-check", "vulnerability",
  "policy-communication",
]);

const batch = JSON.parse(readFileSync(join(ROOT, "ISMS-F-REWRITE-BATCH-1.json"), "utf8"));
const proposed = new Map(batch.rows.map((r) => [r.slug, r.proposed]));
...
if (APPROVED.size !== 14) { console.error("REFUSING: expected 14 approved, have " + APPROVED.size); process.exit(2); }
...
console.log("");
console.log("ISMS-F BATCH 1 -- " + plan.length + " row(s) to write of " + APPROVED.size + " approved");
for (const p of plan) {
  console.log("");
  console.log("  " + p.slug);
...
writeFileSync(join(ROOT, "ISMS-F-BATCH-1-APPLIED.json"), JSON.stringify({
...
console.log("Batch 1 applied: " + plan.length + " row(s).");
