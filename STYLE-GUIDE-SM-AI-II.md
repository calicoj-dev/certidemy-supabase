...
#### The AI calibration — the same test as scenery-swap, and the verdict inverts

> **RECOGNISING THAT A MODEL DOES NOT CHANGE THE ANSWER IS ITSELF A COMPETENCE.** A Scrum
> Master who believes a generated artifact gets a different Definition of Done, or that a
> model-produced number is more objective than a human one, is **wrong** — and knowing
> that is learned. It does not follow from knowing the Guide, because the Guide has never
> heard of the question.

**So "the answer is the same" is a finding**, and it is the calibration shape this
credential already uses everywhere else: `04-04`'s chosen deadline, `04-01`'s stable
environment, `02-09`'s practice that displaces nothing. **The candidate who reports no
defect where none exists is demonstrating the competence, not failing to find one.**

#### The separation that makes this safe

**ITEMS STAY CLEAN.** The external review's argument holds for the examination: an item
that requires noticing a model measures two constructs, and **every D1–D4 item must be
answerable without one.** Nothing about the item bank changes. `SCHEME-SM-AI-II.md` §2.1
states the scope decision and it is unaffected.

**LESSONS MAY CARRY THE CALIBRATION, BECAUSE TEACHING IS NOT TESTING.** A D1–D4 lesson may
include a case in which a model is present and **the correct response is identical to the
non-AI case, with the feedback saying exactly that.** Items are generated from task
scaffolding and the minimally competent candidate definition rather than from lesson
prose, so a lesson case does not reach the bank on its own.

#### One test, two verdicts

| | where | a model is present and the key does not move |
|---|---|---|
| **Scenery-swap** | a **D5 item** | **defect** — the item belongs to its non-AI sibling and the AI is decoration |
| **AI calibration** | a **D1–D4 lesson case** | **the point** — and saying so plainly is the teaching |

**The test is the same and the verdict inverts.** Remove the model, do the work by hand at
the same volume, and see whether the key moves. **In a D5 item an unmoved key means the AI
is decoration. In a D1–D4 lesson case an unmoved key, stated, is what the reader came
for.**

**The feedback has to say it.** A case that includes a model and then simply resolves
correctly teaches nothing about the model — the reader cannot tell whether the AI was
irrelevant or whether the author forgot about it. **The sentence that makes it a
calibration is the one that names what did not change and why.**

#### Where the corpus does this today: nowhere, and that is the finding

**Searched: all 35 authored lessons, every body sentence, for AI, model, generated,
agent, automation, LLM, prompt and their inflections. 31 hits, of which 29 are other
senses** — an *automated scan* in CI, a *four-stance model* from the coaching literature,
*prompt* as a verb, `SM-AI-I` as a credential code. **A model appears in exactly two
places, and neither is a calibration under this rule:**

**`01-03`'s pitfall carries the calibration's CLAIM without its FORM.** It states that
*"The Definition of Done may be relaxed for work generated by an AI tool"* is not in the
Guide, and then the inverse — *"generated work that meets the Definition of Done **is**
part of the Increment, and there is no provision for holding it to a different standard in
either direction."* **That final clause is the calibration exactly.** But it is delivered
as an assertion about the Guide, in a `pitfall` callout, with no scenario and no candidate
response. It tells the reader the answer does not change; it never shows them a situation
in which they might have thought it would.

**`04-03`'s fifth tell is incidental.** A generated draft appears as **one route among
four** to shared words with an empty basis — inherited, templated, skimmed, generated —
and the remedy is the same for all four. That is compatible with the rule and does not
exercise it: the model is an item in an enumeration, not the case the reader is asked to
judge, and nothing says *the answer did not change because it was generated.*

> **So the corpus contains zero deliberate AI calibrations.** This rule opens something
> the 35 lessons have not done, rather than naming something they were already doing —
> which is worth knowing before writing the first one, because there is no worked example
> to copy.
