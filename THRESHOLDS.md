...
| 16 | **`CLAUSE_WORD_FLOOR = 3` distinct references** | `render-gates.mjs` G5 | a lesson whose whole convention rests on one or two references. **The unit is the DISTINCT REFERENCE, not the occurrence** | **yes, and the unit was wrong first.** `03-04` es writes `capitulo 8.1` 17 times -- one reference, the lesson's own subject, named in nearly every paragraph -- against `apartado` for four distinct references. By occurrence capitulo wins 17:9; by reference apartado wins 4:1, which is the convention. A lesson names its subject in every paragraph, so **occurrence counts measure what the lesson is ABOUT, not how it SPELLS things.** Counting references cut the substitutions from 6 to 3 and produced the answer the director had predicted and told me not to assume |
| 17 | **the unanimity escape: 2 distinct references and zero for any other form** | `render-gates.mjs` G5 | nothing -- it is the floor being LOWERED, not raised | **yes, ruled after measurement.** The floor exists to stop noise outvoting noise; it should not overrule a lesson that has never used any other word. `isms-ia-04-02` reads `capitulo`=2 / others=0 at whole level and was abstaining. Both cases are fixtures: `capitulo=2, others=0` holds, `capitulo=2, clausula=1` abstains |
