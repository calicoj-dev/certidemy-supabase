...
**Supersedes:** `AIMS-F_JTA_v1.2.md` (reviewed at commit `fd94355`)
**Stage 3:** **closed 2026-08-06.** Reviewer read ISO/IEC 42001:2023 and declared
the source. Findings absorbed below. **Stage 4 lock is the scheme owner's gate, not
the reviewer's.**
...
## 0b. STAGE 3 FINDINGS — closed 2026-08-06

The reviewer declared reading the PDF, cited clauses, and confirmed the v1.2
corrections against the text. Confirmed independently: task 2.7 against 6.1.2 and
6.1.4; the 38-control count by per-category arithmetic reaching the same total; the
six-category role taxonomy at 4.1 NOTE 1; the climate-change `shall`; the clause 8
restatement; Annex B's normative marking; Annex D.2's integration language.

### Finding — clause 6.3 had no home. Accepted.

**6.3 Planning of changes** is a one-sentence `shall`: when the organization
determines the need for changes to the AIMS, the changes are carried out in a
planned manner. No task owned it.

**This is ISMS-F's Finding 3 with the excuse removed.** 27001 hid its 6.3 by
omitting it from its own contents page. **42001's contents page lists 6.3
plainly.** D2 was mapped to clauses 4–6 and a clause in that range was skipped.

**Placement — the reviewer's suggestion was declined.** It proposed task 3.4 or
3.8. Both sit in D3, which owns clauses 7–8, and **3.4's `change-control-ai` covers
clause 8.1** — changes to *operations*, including review of *unintended* changes.
**6.3 is changes to the management system itself.** Different clause, different
object, different domain.

**Resolved as a concept on task 2.3**, which already owns clause 6.2, in the domain
mapped to clauses 4–6. Same shape as ISMS-F's resolution: a concept, not a new
task, so **the blueprint does not move.** Concepts 153 → **154**; mentions 156 →
**157**.

### Finding — task 4.6 promoted Annex B guidance to control text. Accepted.

v1.2 wrote *"meaningful human oversight with authority to override"* as though it
were a Table A.1 control. **A.9.2 and A.9.3 say only that the organization defines
and documents processes, and identifies and documents objectives, for responsible
use.** The fairness/accountability/transparency list and the override-authority
language are **B.9.3**, which uses *should* and introduces its list with *some
objectives include*.

**One nuance the review understated: Annex B is normative.** So this is not
guidance-versus-requirement. It is **`should` versus `shall`, and illustrative
versus exhaustive** — and that is exactly the distinction a generator flattens.

### Finding — task 2.7's modal. Accepted as sharpening.

The `shall` was already carried (*"must be considered"*). **6.1.4 is now named**, so
the requirement cannot be read as an inference.

### Two review statements not taken at face value

**"No additional false attributions were found in the K fields examined."** The
hedge is load-bearing — it does not say all 35 tasks. **Treat the attribution sweep
as spot-checked, not exhaustive.** This is why §7 item 6 survives regardless.

**"The JTA is lockable."** Stage 3 closing is what a review determines. **Stage 4
lock is the scheme owner's gate.** Recorded because `load-jta-i18n.mjs --approve`
once printed *APPROVED - Grok has signed off* when no review had run.
...
| Criticality | high · Frequency | occasional |
| Bloom | `2_understand` · Exam scope | yes |
| Concepts | `top-management-commitment`, `ai-policy`, `ai-objectives`, `policy-vs-objective`, `objectives-measurable`, `planning-of-changes` |
...
evaluated. **Clause 6.3 adds a one-sentence requirement that is easy to miss:
where the organization determines a need for changes to the AI management system,
those changes are carried out in a planned manner.** That concerns changes to the
management system itself — distinct from clause 8.1's control of operational
changes (task 3.4).
**S:** Distinguish a policy statement from an objective, and an AIMS change from an
operational change.
**A:** Expectation that leadership is evidenced, not asserted.

> **`planning-of-changes` added in v1.3** from Stage 3. Clause 6.3 was untaught.
> Added as a concept rather than a task, so the blueprint does not move.
...
societies. **The impact assessment is an input, and the link is a requirement:
clause 6.1.4 states that the organization shall consider the results of the AI
system impact assessment in the risk assessment, with a reciprocal NOTE at 6.1.2
permitting its use when assessing consequences.** What actually separates them is four things: **anchoring** (risk to
...
**K:** The Table A.1 use controls require the organization to **define and document
processes** for responsible use and to **identify and document objectives** guiding
it; intended-use controls require the system to be used per its documentation;
relationship controls allocate responsibilities across the organization, partners,
suppliers, customers and third parties, and consider customer expectations.
**The familiar content — fairness, accountability, transparency, explainability,
reliability, safety, robustness, privacy, accessibility, and meaningful human
oversight with authority to override — is Annex B guidance at B.9.3, which says
*should* and offers the list as examples rather than a required set.** Annex B is
normative, so this is not optional reading; but **a `should` and an illustrative
list must not be taught as a `shall` and an exhaustive one.** Which controls apply
depends on the role held (task 1.2).
...
**157 concept mentions · 154 distinct concepts · 157 `task_concepts` link rows.**

*(v1.3: D2 gained `planning-of-changes` on task 2.3 for clause 6.3. Task count and
blueprint unchanged.)*
...
**Scaffold arithmetic:** 154 rows in `concepts`, 157 rows in `task_concepts`. If
...
2. ~~Stage 3 external review.~~ **Closed 2026-08-06** — see §0b. Three findings,
   all absorbed. **Stage 4 lock remains the scheme owner's decision.**
3. **Concept deduplication pass** before scaffold migration **176**: **154
   concepts, 157 links**, three intentional reuses. Slug change in v1.2 —
   `certifiable-management-system` → `management-system-certification-basis`.
   New slug in v1.3 — `planning-of-changes` on task 2.3.
