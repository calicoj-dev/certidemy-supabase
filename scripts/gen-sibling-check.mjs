...
if (flagged.some((f) => !f.slug || !f.en_description)) {
  console.error("a flagged row carries no slug or no English -- the file shape changed"); process.exit(2);
}
...
writeFileSync(OUT, JSON.stringify({
  ...
  how_to_work_this_file: [
    "Read en_description, then flagged_translation, then sibling_translation.",
