-- Migration 009: unpublish NULL until its blueprint is populated.
--
-- NULL's certifications row exists with is_published = true, but its JTA
...
-- getCertByCode return null for it, so /learn/NULL/* cleanly 404s
...
-- Sanity check: NULL's row should now read as unpublished. Fail loud if it
...
begin
  ...
  from public.certifications where code = 'NULL';
  if is_pub is null then
    -- No NULL row at all; surprising but not a failure.
    raise notice 'NULL row not found; nothing to unpublish.';
  elsif is_pub then
    raise exception 'NULL is still published after migration; update did not apply.';
  end if;
end $$;
