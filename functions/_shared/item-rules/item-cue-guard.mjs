...
// ---------------------------------------------------------------------------
// NO process.env HERE, AND THAT IS THE WHOLE REASON THIS FILE MOVED.
//
// These three used to read process.env AT MODULE SCOPE. That fires on IMPORT,
// so it broke a Deno caller that never touched the cue config at all -- the one
// thing standing between this module and the edge runtime, measured 2026-09-18:
// `deno check` passed on all five rule modules and `deno run` died on this
// line alone.
//
// The env override was never a RULE. It is a local debugging affordance for the
// generator scripts, so it now lives with them, in scripts/lib/item-cue-guard.mjs,
