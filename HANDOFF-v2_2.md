# HANDOFF-v2.2 — Catalog Discovery + Cert i18n
...
1. **Cert detail page i18n.** `/[locale]/certifications/[code]` has the identical
   English-on-Spanish bug, in a worse location. Needs the same
   `certification_i18n` join. Also needs the 12 long-form **description**
   translations, which were deliberately deferred (the catalog only reads
   `claim`). Load via the API script, never the editor.

2. **Enrollment shift to opt-in — its own session.** Goal: a fresh user has no
   certs assigned and self-enrols, ideally through a navigator-style surface.
   **This is an access-control change, not a UI task.** First move is dumping
   the current auto-assign mechanism (trigger? console default? seed script?)
   and the learn-route RLS, because the dashboard / exam / quiz pages likely
   assume an enrollment row always exists and will throw when it doesn't.
   Will need a migration — DB chat holds the pen.

3. **Claims into the scheme docs.** Six `SCHEME-<CODE>.md` files.

4. **Optional third field:** a one-sentence "who this is for" per cert. Deferred
   — it would have meant authoring 36 sentences instead of 18 before anything
   shipped.
...
Interaction: ~7.5/10 and shipping-grade. Content: was 4/10 before this session
because of the English-copy problem; the catalog is now fixed in three languages
but **the cert detail page is not**, and that is the page that closes the sale.
Item 1 above is the remaining gap to "commercially viable."
